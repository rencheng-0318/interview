...
**问题背景**：初始尝试使用并行处理加速索引构建，但在实际测试中发现 embedding 服务在高并发下会出现大量失败（2400 个文档中有 1798 个失败）。
...
| 方案 | 优势 | 劣势 | 适用场景 |
|------|------|------|---------||
| **串行 (选择)** | 简单可靠；零资源争抢；100% 成功率 | 速度慢（~8-10 min）；CPU 利用率低（~10%） | **当前规模（~5000 chunks）** |
| 分批并行 | 理论速度快（~10x） | 需精细调优 BATCH_SIZE；embedding 服务可能过载 | 大规模且基础设施完善 |
| 完全并行（全并发） | 理论最快 | OOM 风险；压垮下游服务；调试困难；故障隔离差 | 不适合当前架构 |

**为什么放弃并行？**

在测试中实现的 `BATCH_SIZE = 10` 分批并行方案出现以下问题：

1. **Embedding 服务过载**：FastAPI + `run_in_threadpool` 架构下，同时处理多个请求导致线程池耗尽、响应变慢
2. **大量失败**：2400 个文档中有 1798 个 embedding 失败（失败率 75%）
3. **故障恢复复杂**：需要在批次间增加等待和重试逻辑，进一步增加代码复杂度
4. **边际收益有限**：对于单次索引任务（仅在建库时运行一次），10 分钟可接受

**权衡过程：**

| 批量大小 | 测试结果 | 成功率 | 评价 |
|---------|---------|--------|------|
| 10（原始尝试） | embedding 服务明显变慢 | ~25% | ❌ 不可接受 |
| 5（降低后测试） | 未完整测试 | ? | ⚠️ 可能需要但增加复杂度 |
| 1（串行，选择） | 稳定可靠 | ~100% | ✅ 最佳平衡 |

**维护性考量：**
- 每个文档保持独立事务，失败不影响其他文档（故障隔离）
- 增量索引支持：下次运行只会处理新增/变更的文档
- 日志清晰：`embedding chunks=3914 documents=1998` 易于调试
- 代码简洁：无需 semaphore、batch 管理、并发错误处理

**性能预期：**
- 2400 文档 / ~4700 chunks：约 8-10 分钟
- 执行频率：仅在数据重建或首次建库时运行（非热路径）
- 优化方向：若扩展到 10k+ 文档，可考虑为 embedding 服务添加连接池限制
